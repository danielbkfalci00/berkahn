...
export function ArchitectMetrics({ architect }: Props) {
  ...
  // Override leve: arquitetos sem perfil de "escritório consolidado" (ex: emergentes)
  // podem definir customMetrics com rótulos/valores próprios (string), sem CountUp.
  const custom = architect.customMetrics?.length ? architect.customMetrics : null;
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-black-10">
          {custom
            ? custom.map((stat, i) => (
                ...
                >
                  <p className="font-heading text-3xl md:text-4xl lg:text-5xl font-extralight tracking-tight text-black">
                    {stat.value}
                  </p>
