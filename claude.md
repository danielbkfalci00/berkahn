...
| `conteudo/check-lote-linkedin.mjs` | Confere um CONJUNTO de posts do LinkedIn contra fórmula de lote, que o verificador por post não enxerga por construção: distribuição de aberturas em pergunta, CTA abrindo com verbo em primeira pessoa do plural, repetição da primeira palavra de hook e de CTA, e folga de extensão. Criado em 2026-09-05, quando dez posts passaram individualmente com exit 0 e sete abriam o CTA com a mesma forma. Uso: `node scripts/conteudo/check-lote-linkedin.mjs --dir=<pasta> --glob=<prefixo>` |
...
| `admin/feedback.mjs` | CLI do mural de feedback do admin (migration 034): `listar [--status=aberto|implementado|todos]`, `ver <id>`, `responder <id> --arquivo=<path>` (texto só de arquivo, grava como "Claude", origem `cli`) e `status <id> implementado|aberto [--nota="PR #123"]`. `--dry-run` em toda escrita, `--json` na leitura. Service role; sem a 034 aplicada sai com código 2 e mensagem clara. Doc: `Berkahn-Vault/60-arquitetura/admin-feedback.md` |
