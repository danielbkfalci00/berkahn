...
console.log("validarNovoFeedback");
{
  ...
  const semTitulo = m.validarNovoFeedback({ titulo: "", categoria: "bug", descricao: "Não consigo enviar feedback" });
  ok("aceita descrição sem título", semTitulo.ok);
  ok("deriva título da descrição", semTitulo.ok && semTitulo.valor.titulo === "Não consigo enviar feedback");
  ok("recusa formulário vazio", !m.validarNovoFeedback({ titulo: "", categoria: "bug", descricao: " " }).ok);
