...
export async function dispatchLeadPushNotifications(): Promise<PushDispatchResult> {
  ...
  if (!publicKey || !privateKey) {
    return { claimed: 0, sent: 0, failed: 0, disabledSubscriptions: 0, configured: false };
  }

  webpush.setVapidDetails(subject, publicKey, privateKey);
  const supabase = createServiceClient();
  const [{ data: notifications, error: claimError }, { data: subscriptions, error: subscriptionsError }] = await Promise.all([
