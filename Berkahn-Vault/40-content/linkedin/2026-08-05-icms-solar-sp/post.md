---
tipo: linkedin-post
criado: 2026-08-05
atualizado: 2026-08-11
tags:
  - project/linkedin
  - status/draft
  - domain/sustentabilidade
  - domain/normas
  - domain/lsf
ai_summary: "Post do artigo de ICMS solar em SP. Recorte único: a pressa que o mercado vende não trava nada, nem a isenção nem o Fio B, e a alavanca que realmente move a conta é decisão de planta. Pilar: previsibilidade. Público: residencial alto padrão em São Paulo."
status: draft
projeto: linkedin
slug: 2026-08-05-icms-solar-sp
data_publicacao: 2026-08-05
contextos_aplicados:
  - berkahn-brand
  - copy-sem-travessao
  - seo-aeo-strategy
artigo_slug: isencao-icms-energia-solar-sp
plataforma: linkedin
hashtags:
  - LightSteelFrame
  - SteelFrame
  - EnergiaSolar
  - ConstrucaoIndustrializada
  - EficienciaEnergetica
---

> **Artigo blog**: [[isencao-icms-energia-solar-sp]] · **Hub**: [[linkedin]] · **Briefing imagem**: [[40-content/linkedin/2026-08-05-icms-solar-sp/briefing-imagem|briefing-imagem]]

Se você tem casa em São Paulo e já pensou em energia solar, provavelmente ouviu que precisa instalar antes de dezembro.

A isenção de ICMS acaba mesmo em 31 de dezembro. Só que instalar antes não protege ninguém. Quando ela cai, cai para todo mundo, inclusive para quem tem painel no telhado desde 2019. Com o Fio B é a mesma coisa.

Correr, então, muda pouco. O que muda bastante é mais sem graça, que é usar a energia na hora em que ela é gerada. Programar a bomba da piscina para o meio-dia rende mais no ano do que a pressa toda.

Quem ainda está projetando decide isso de graça, no desenho. Em Light Steel Frame o telhado e o caminho do eletroduto entram no modelo antes da obra. Com a casa pronta, vira reforma.

Tem um artigo com a conta aberta no nosso blog.

https://www.berkahn.com.br/atualidades/isencao-icms-energia-solar-sp?utm_source=linkedin&utm_medium=social&utm_campaign=post-organico

#LightSteelFrame #SteelFrame #EnergiaSolar #ConstruçãoIndustrializada #EficiênciaEnergética

---

<!-- vault-rodape-v1 -->
**Relacionado**: [[isencao-icms-energia-solar-sp]] · briefing visual em `briefing-imagem.md`

**Contexto aplicado**: [[berkahn-brand]] · [[copy-sem-travessao]] · [[seo-aeo-strategy]]

**Nota de recorte**: o post usa apenas um dos quatro achados do artigo (a falsa trava dos dois prazos). A decomposição ICMS contra Fio B e o cronograma reverso de quem está em obra ficam guardados como recortes para posts futuros do mesmo artigo.
