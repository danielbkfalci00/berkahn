---
tipo: context
criado: 2026-04-13
atualizado: 2026-05-21
tags:
  - ai/context
  - project/site
  - project/blog
  - project/linkedin
ai_summary: Identidade da marca Berkahn — construtora LSF "Erguendo o amanhã". 3 pilares (leveza/previsibilidade/limpeza). Voz "engenheiro experiente". Regras rígidas contra vícios de IA. LSF ≠ Steel Frame.
status: active
escopo: berkahn
---

# Berkahn — Identidade de Marca

## Dados da Empresa

```
BERKAHN
Erguendo o amanhã

contato.berkahn@gmail.com
+55 (11) 96641-5742
São Paulo, SP - Brasil
CNPJ: 39.455.932/0001-64
berkahn.vercel.app

Instagram: @berkahn
LinkedIn: /company/berkahn
```

## Posicionamento

A Berkahn é uma construtora especializada em **Light Steel Frame (LSF)** — sistema construtivo com perfis leves de aço galvanizado formados a frio. Atende obras residenciais e comerciais de alto padrão.

### 3 Pilares
A Berkahn existe para trazer três coisas para a construção civil:
1. **Leveza** — estruturas mais leves, processos mais ágeis
2. **Previsibilidade** — cronograma e orçamento cumpridos
3. **Limpeza** — obra seca, sem desperdício

### Segmentos
- **Residencial alto padrão** — ticket médio R$ 500k-2M
- **Comercial** — galpões, escritórios, lojas
- **Industrial** — fábricas, centros de distribuição

### Diferenciais
- Certificação LSF
- Processo 360° (conceito → chaves)
- Expertise em LSF híbrido (steel frame + concreto)
- 20+ anos de experiência dos sócios

## Voz da Marca

### Tom
- Educativa, leve e com personalidade
- Como um engenheiro experiente compartilhando algo interessante, não dando aula
- Profissional mas conversacional
- Confiante sem arrogância
- Técnica mas acessível — traduz complexidade sem subestimar o leitor

### Pronomes
- Voz é "nós/a gente" como empresa
- NÃO é "a Berkahn fez" (terceira pessoa distante)
- SIM é "a gente publicou", "no nosso blog", "nós trabalhamos com"
- Em artigos do blog: voz impessoal no corpo, "nós" só na abertura e CTA

### Vícios de Linguagem Proibidos

Estes padrões denunciam texto gerado por IA. Evitar sempre (regra completa em [[copy-sem-travessao]]):

- Travessão como recurso estilístico ("e isso muda tudo — literalmente")
- Dois pontos como recurso estilístico em parágrafos
- "Quase ninguém fala/sabe sobre isso"
- "Você sabia que...?", "Nos últimos anos...", "Vamos falar sobre..."
- "Não é apenas X, é Y" ou "Mais do que X, é Y"
- "E" ou "Mas" isolados no início de parágrafo como efeito dramático
- Superlativos vazios: "incrível", "revolucionário", "game changer", "divisor de águas"
- Frases curtas com ponto final dramático em sequência (tom de manifesto)
- "Genuinamente", "honestamente", "de forma straightforward"
- Qualquer retórica artificial ou clichê corporativo
- "Na Berkahn, a gente..." como abertura de parágrafo (virou muleta)
- Endereçar público antes de pergunta ("Pra quem gerencia obra:", "Arquitetos:")

**Regra**: Se a frase parece gerada por IA, reescrever até parecer humana.

## Terminologia: Light Steel Frame vs. Steel Frame

A Berkahn trabalha com **Light Steel Frame (LSF)** — perfis leves de aço galvanizado formados a frio. **Steel Frame** (aço laminado pesado, perfis de grande porte) é um sistema construtivo diferente.

Detalhes técnicos em [[steel-frame-domain]].

### Regras de uso
- Primeira menção: sempre "Light Steel Frame" por extenso
- Menções seguintes: pode usar "LSF" como abreviação
- NUNCA usar "Steel Frame" sozinho como sinônimo de LSF no corpo do texto
- Exceção: hashtags (#SteelFrame tem valor de alcance)
- Se o dado é específico do LSF, referenciar como "Light Steel Frame" ou "LSF"
- Se o título de um artigo usa "Steel Frame", pode reproduzir como está, mas no corpo manter "LSF"

## ICP (Ideal Customer Profile)

### Público primário
- Famílias em São Paulo decidindo como construir a própria casa
- Ticket alto, buscam qualidade e previsibilidade
- Dores: medo de estourar orçamento, atraso, sujeira, insegurança sobre sistema construtivo

### Público secundário
- Arquitetos e engenheiros buscando informações técnicas sobre LSF
- Incorporadores e desenvolvedores comerciais

### Dores reais do público
- Medo de obra estourar orçamento
- Medo de atraso interminável
- Frustração com desperdício e sujeira de obra
- Insegurança sobre qual sistema construtivo escolher
- Receio de decisão de centenas de milhares de reais sem informação suficiente
- Obras que viram sinônimo de estresse e imprevisibilidade
- Ter que aceitar decisões do incorporador quando compra pronto

## Visual

Detalhes em [[design-principles]].

- **Paleta**: Preto/Branco/Cinzas + Off-white #F4F2EC
- **Tipografia**: Manrope (Google Fonts)
- **Ícones**: Line/Outline, stroke 1-1.5px, rounded
- **Estilo**: Minimalista, premium, muito espaço em branco
- **Logo**: Variações em preto e branco

## Referências

- Design system: [[design-principles]]
- Domínio LSF: [[steel-frame-domain]]
- SEO/AEO: [[seo-aeo-strategy]]
- Sistema de apresentações: [[presentation-system]]
- Regra anti-travessão: [[copy-sem-travessao]]
