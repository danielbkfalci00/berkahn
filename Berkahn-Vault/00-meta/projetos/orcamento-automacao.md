...
tags:
  ...
  - status/published
ai_summary: Hub do Gerador de Estimativa Preliminar Premium — automação de PDFs de orçamento via admin Berkahn (form ou planilha-modelo). Sprints 1-5 entregues (fundação + renderer + 12 componentes + form wizard 5 steps + upload XLSX/CSV + filtros lista + soft delete UI + hints XLSX + cookie LSF). MVP completo. Pendência única: Bruno autorizar supabase gen types pra limpar 6 @ts-expect-error. Plano em ~/.claude/plans/eu-preciso-seguir-com-optimized-starlight.md.
status: active
...
- [ ] **Ação Bruno**: `npx supabase login` + rodar `supabase gen types typescript --project-id sfqaknxomxwmviarpwfy > types/supabase-db.ts` e remover 6 `@ts-expect-error` (frente D do Sprint 5 — bloqueada por auth CLI)
